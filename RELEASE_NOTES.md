...
## 🔧 Changed: macOS bundle identifier

The application's macOS bundle identifier has been changed from `com.termpane` to `com.termpane`. Application data in `~/Library/Application Support/TermPane/` is **unaffected** (the path is based on `productName`, not the bundle ID) — preferences and saved state survive the upgrade.

Because macOS treats apps with different bundle identifiers as separate apps, a previously installed `0.2.0` build won't be replaced in-place by `0.2.1`. Drag the old `TermPane.app` to the trash before installing the new one.
